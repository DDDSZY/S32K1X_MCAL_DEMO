...
#define MCU_IPW_SW_PATCH_VERSION_C              6
...
FUNC( void, MCU_CODE) Mcu_Ipw_InitClock(P2CONST( Mcu_ClockConfigType, AUTOMATIC, MCU_APPL_CONST) Mcu_pClockConfigPtr)
{
    ...
    */
    ...
    if (
        (SCG_SYS_CLK_NOT_UNDER_MCU_CONTROL != Mcu_pClockConfigPtr->SCG_pClockConfig->u32RunClockControlConfiguration) && \
        (SCG_SCS_SPLL_U32 != (Mcu_pClockConfigPtr->SCG_pClockConfig->u32RunClockControlConfiguration & SCG_SCS_SPLL_U32))
       )
    {
#ifdef MCU_HSRUN_MODE_NOT_SUPPORT
#if (MCU_HSRUN_MODE_NOT_SUPPORT == STD_OFF)
        if (SCG_SCS_SPLL_U32 != (Mcu_pClockConfigPtr->SCG_pClockConfig->u32HSRUNClockControlConfiguration & SCG_SCS_SPLL_U32))
        {
#endif /*(MCU_HSRUN_MODE_NOT_SUPPORT == STD_OFF)*/
#endif 
            /* get the current System Reset Interrupt Enable settings */
            u32SystemResetIsrConfig = Mcu_RCM_GetCurrentSystemResetIsrSettings();
